...
int main(int argc, char *argv[]) {
    ...
    for (int problem = 0; problem < numProblems; ++problem) {
        ...
        auto *curProblemData = new ProblemData();
        curProblemData->outputVisualization = outputVisualization;
        curProblemData->constructPathForVisualization = constructPathForVisualization;
        ...
        if(TIME_STEPS >= 2)
        {
            simulate_waves(*curProblemData, 2);
        }
        ...
        for (int t = 2; t < TIME_STEPS; t++) {
            #pragma omp parallel sections
            {
                ...
                #pragma omp section
                {
                    ...
                    if (findPathWithExhaustiveSearch(*curProblemData, t, path)) {
                    ...
                    if (outputVisualization) {
                        VideoOutput::writeVideoFrames(path, *curProblemData);
                    }
                }
            }
